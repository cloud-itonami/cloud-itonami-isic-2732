(ns harnessworks.operation
  "OperationActor -- one wire/cable/harness-manufacturing operation =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (Harness Advisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Cable-Integrity
  ...
  One graph run = one wire/cable/harness-manufacturing operation
  ...
  (:require [langgraph.graph :as g]
            ...
            [harnessworks.harnessworksadvisor :as harnessworksadvisor]
...
(defn build
  ...
  `harnessworks.store/Store`).
  opts:
    :advisor      -- a `harnessworks.harnessworksadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (harnessworksadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Harness Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (harnessworksadvisor/-advise advisor store request)]
            {:proposal p :audit [(harnessworksadvisor/trace request p)]})))

      ;; Cable-Integrity Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
